...
export default function Home() {
  ...
  return (
    <>
      <Head>
        <title>FunCaption - Instagram Captions with Regional Pride</title>
        <meta name="description" content="Create viral Instagram captions with regional pride" />
      ...
      <div className="min-h-screen bg-gradient-to-b from-purple-900 via-black to-black text-white">
        ...
        <section className="container mx-auto px-4 py-20 text-center">
          ...
          <p className="text-lg md:text-xl text-gray-300 mb-6 max-w-3xl mx-auto">
            Create viral Instagram captions — with your regional pride & vibe!
          </p>
          <p className="text-base text-purple-300">
            ✨ 18 Regional Vibes | Native Scripts | Pride Lines ✨
          </p>
        </section>

        {/* Regional Badges */}
        <section className="container mx-auto px-4 py-8">
          <h3 className="text-xl font-bold text-center mb-4">Your Culture. Your Language. Your Caption.</h3>
          <div className="flex flex-wrap gap-2 justify-center max-w-5xl mx-auto">
            {regionalVibes.filter(v => v !== 'None').map((vibe, i) => (
              <span 
                key={i} 
                className="bg-purple-800/40 border border-purple-500/50 px-3 py-1 rounded-full text-xs font-medium"
              >
                {vibe}
              </span>
            ))}
        ...
        <section className="container mx-auto px-4 py-12 max-w-2xl">
          <div className="bg-gradient-to-br from-purple-900/40 to-black border border-purple-500/30 rounded-3xl p-6 md:p-8">
            <h3 className="text-2xl font-bold mb-6 text-center">Generate Your Caption</h3>
            ...
            <form onSubmit={handleGenerate} className="space-y-4">
              ...
              {/* Mood */}
              <div>
                <label className="block text-sm font-medium mb-2">😎 Choose Mood</label>
                <select 
                  value={mood}
              ...
              {/* Regional Vibe */}
              <div>
                <label className="block text-sm font-medium mb-2">🏛️ Regional Vibe (Optional)</label>
                <select 
                  value={regionalVibe}
              ...
              <button 
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 disabled:opacity-50 disabled:cursor-not-allowed text-white font-bold py-3 rounded-lg transition"
              >
                {loading ? '✨ Generating...' : '🚀 Generate Captions (Free!)'}
              </button>
            </form>

            {/* Results - APPEARS BELOW THE FORM */}
            {captions.length > 0 && (
              <div className="mt-8 space-y-4">
                <h4 className="text-xl font-bold text-center">Your Captions Are Ready! 🎉</h4>
                
                {captions.map((captionObj, i) => (
                  <div key={i} className="bg-gradient-to-br from-purple-800/20 to-pink-800/20 p-4 rounded-xl border border-purple-500/40">
                    {/* Native Language Caption */}
                    ...
                    {/* Copy Button */}
                    <button 
                      onClick={() => copyCaption(captionObj.caption, i)}
        ...
        <footer className="border-t border-purple-500/30 py-6 mt-12">
          <div className="container mx-auto px-4 text-center text-gray-400 text-xs">
            <p>FunCaption © 2025 - Celebrate Your Regional Pride 🇮🇳</p>
          </div>
        </footer>
      </div>
    </>
